...
title('Filtered image4');

%calculate temporal gradient
offset = 90; %use to move double pixel value to integer range

temporal_gradient = double(filtered_image5)-double(filtered_image4);
figure(2);
...
for i=2:266
    for j=1:534
        e_x(i-1,j) = double(filtered_image4(i,j))-double(filtered_image4(i-1,j));
    end
end
figure(3);
...
for i=1:266
    for j=2:534
        e_y(i,j-1) = double(filtered_image4(i,j))-double(filtered_image4(i,j-1));
    end
end
figure(4);
